...
function rest_of_test() {
	...
	*/
	...
	let i = 0;
	let hash_set = []
	console.log("looping: " + cache_count)
	while ( i++ < cache_count ) {
		hash2++;
		el_diff++
		hashes.push(`${hash2}-${el_diff}`)
		el_data = `GEEE ${i} wolly gollies oglum`
		hash_set.push([hash2,el_diff,el_data])
	}

	console.log("Setting many")

	let status = shm.set_many(arr2D2.key,hash_set)

	console.log("DONE Setting many")
